...
:heavy_check_mark: 또한, 프레임워크의 도움없이, 레이어드 아키텍처 (with 확장된 mvc 패턴) 기반으로 구현하는 과정을 체험한다.
...
해당 프로젝트에서는 회원 정보 중 비밀번호 저장 시에 별도의 암/복호화 기능을 구현하지는 않았습니다.
(실제 프로젝트에서는 db에 비밀번호 보관 시에 비밀번호 암호화 필수)
...
로그인의 경우 실제 로그인 기능을 구현하지 않고, 단순 DB 값 비교로 진행하였습니다. 
