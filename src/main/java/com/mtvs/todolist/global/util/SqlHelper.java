...
public final class SqlHelper {
    ...
    private static void loadQueries() {
        try {
            ...
            InputStream inputStream = SqlHelper.class.getClassLoader().getResourceAsStream("SqlMapper.xml");

            // InputStream이 null인 경우, 즉 파일을 찾지 못한 경우 예외 발생
            if (inputStream == null) {
                throw new RuntimeException("SqlMapper.xml 파일을 찾을 수 없습니다.");
            }
            ...
            for (int i = 0; i < nodeList.getLength(); i++) {
                ...
                String sql = queryElement.getTextContent().strip();
